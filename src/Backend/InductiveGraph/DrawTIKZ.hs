...
drawNodeRelativeTo
  :: Node -> (Float,Float) -> String -> String
drawNodeRelativeTo n d expl =
  [i| \\node [draw=none] at #{rpos n d} {#{expl}};|]

drawBarRelativeTo :: Node
                  -> (Float,Float)
                  -> Float
                  -> Float
                  -> Int
...
drawBarRelativeTo n d dim dimy ix v =
  let (dx,dy) = d
      ...
      sty =
        "rectangle, inner sep=0pt, draw=none, anchor=south, minimum width=" ++
        mw
...
drawNodes o b expl =
  let
      ...
      addProb =
        case mf of
          Just f ->
            let dc (_,n,_,_) s =
                  drawChart n Lft (nodeProb (toProbBDD f b) n) ++ s
            in ufold dc "" b
          Nothing -> ""
      pfx =
        drawNodeRelativeTo (findRootNode b)
                           (0,1)
                           expl ++
        addProb
      accumulateOnContext
        :: Context NodeLabel EdgeLabel -> String -> String
      accumulateOnContext c a =
        let nid = node' c
            nlb = lab' c
            edgesi = inn b nid
            edgeso = out b nid
  ...
  in prologue ++ graph ++ epilogue pfx

data Options =
  Opt {
  _drawArrows :: Bool,
  _variablesList :: [String],
  _probabilities :: Maybe (String -> Float)
  }

defaultOptions = Opt True [] Nothing

$(makeLenses ''Options)

drawPdfInfoSheet o e n =
  let bdd = buildBDD (_variablesList o) (unE e)
  in plotTikz n $ drawNodes o bdd (printAsEquation $ unE e)
